/**
 * Página de Gerenciamento de Artigos
...
import { FileText, Plus, Trash, Pencil, Search, Star, Filter, ChevronDown } from "lucide-react";
...
const AdminArticles = () => {
  const { articles, addArticle, removeArticle, updateArticle, categories, addCategory, removeCategory } = useBlog();
  ...
  console.log('AdminArticles - Rendered with articles:', articles); // Debug log
  ...
  const handleRemoveCategory = () => {
    if (categoryToRemove) {
      const articlesWithCategory = articles.filter(article => article.category === categoryToRemove);
      if (articlesWithCategory.length > 0) {
        toast.error(`Não é possível remover a categoria "${categoryToRemove}" pois existem ${articlesWithCategory.length} artigos vinculados a ela.`);
      } else {
  ...
  const authors = React.useMemo(() => {
    const uniqueAuthors = new Set(articles.map(article => article.authorName));
    return Array.from(uniqueAuthors).filter(Boolean);
  }, [articles]);
  ...
  const filteredArticles = (articles || []).filter(article => {
    try {
      ...
      const matchesAuthor = selectedAuthor === "all" || article.authorName === selectedAuthor;
          ...
          <div className="container-custom">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                <Dialog open={isRemovingCategory} onOpenChange={setIsRemovingCategory}>
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Selecione a categoria que deseja remover. Não será possível remover categorias que possuem artigos vinculados.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                      <Select
                        value={categoryToRemove}
                        onValueChange={setCategoryToRemove}
                      >
                        <SelectTrigger>
                ...
                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                  ...
                  if (!open) {
                    // Resetar o formulário e estados quando fechar o modal
                    form.reset({
                      title: "",
                      excerpt: "",
                      content: "",
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                ...
                <div className="flex gap-4 items-center flex-wrap">
                  ...
                  <Select value={selectedAuthor} onValueChange={setSelectedAuthor}>
                    <SelectTrigger className="w-[180px]">
                      <SelectValue placeholder="Todos autores" />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-xs text-gray-500 flex items-center gap-1">
                          <span className="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">
                            Autor: {article.authorName || "Não especificado"}
                          </span>
                        </span>
                      </div>
                ...
                <div className="col-span-full">
                  <div className="text-center py-12 bg-gray-50 rounded-xl">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      {searchTerm || selectedCategory || selectedAuthor || showFeaturedOnly
